...
void init(void)
{
	...
	view_mat = lookAt(0, 4, 0, 
		    0, 0, 0,
		    0, 1, 0);

	modelview_mat = Mult(view_mat,trans_mat);
	result 	      = Mult(proj_mat,modelview_mat);

  	glUniformMatrix4fv(glGetUniformLocation(program, "myMatrix"), 1, GL_TRUE, result.m);	
   	glUniformMatrix4fv(glGetUniformLocation(program, "myProjectionMatrix"), 1, GL_TRUE, projectionMatrix);
